...
class WPTEnv(discrete.DiscreteEnv):
    ...
    def __init__(self):
        ...
        self.receiver_position=generate_receivers()
        self.code_book=build_code_book()

        self.prob_LOS = 1 # Probability of LOS transmission
...
#                (pt_interferer <= self.max_tx_power_interference) , (pt_interferer >= 0) , (received_ue2_sinr >= self.sinr_target))
    ...
    def generate_receivers(self):
        ...
        receiver_positions=np.random.randint(1,29,size=(self.K,2))
        return receiver_positions

    def build_code_book(self):
        code_book=[]
        M=self.M
        N=self.N
        theta_base=math.pi/N
        for i in range(N):
            f=compute_bf_vector(M,theta_base*i)
            code_book.append(f)
        return code_book
...
#        print ('Warning: channel gain is {} dB.'.format(10*np.log10(LA.norm(h, ord=2))))
    ...
    def _compute_rf(self, x_tx, y_ue, pt_bs1, pt_bs2, is_ue_2=False):
